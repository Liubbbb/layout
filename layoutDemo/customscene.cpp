...
void CustomScene::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if(findSelectItem())
    {
        ...
        item->itemInf.horizontal_position = qRound(select->scenePos().x()*wStretch); //更新item信息
        item->itemInf.vertical_position = qRound(select->scenePos().y()*hStretch);
        item->itemInf.width = qRound(rect.width()*wStretch);
        item->itemInf.height = qRound(rect.height()*hStretch);
        mul.createWindow(item->itemInf); // 发送创建窗口命令 = 更新窗口
    }
    setCursor(Qt::ArrowCursor);
    selectMode = none;
