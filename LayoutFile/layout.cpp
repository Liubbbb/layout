...
void layout::createLayoutMenu()
{
    ...
    QAction *applyAction = new QAction(tr("应用"),this);
    connect(applyAction,SIGNAL(triggered(bool)),this,SLOT(applayLayout()));
...
void layout::applayLayout()
{
    StLayoutInf layoutInformation;
    mul.retriveLayoutInfo(layoutInformation,selectButton->objectName());
    emit sendLayoutInfo(layoutInformation);
}

void layout::clickRightButton()
{
    qDebug()<<"右键点击";
}
